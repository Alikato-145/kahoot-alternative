...
export default function HostDashboard() {
  ...
  async function start(quiz: Quiz) { try { const session = await quizApi.start(quiz.id); router.push(`/host/game/${session.sessionId}`) } catch (actionError) { setError(actionError instanceof Error ? actionError.message : 'เริ่มเกมไม่สำเร็จ') } }
