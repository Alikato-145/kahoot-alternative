...
export type Quiz = { id: string; title: string; description: string; coverImageUrl: string | null; questions: Question[] }
...
type QuizRow = RowDataPacket & { id: string; title: string; description: string; cover_image_url: string | null }
...
function toQuiz(row: QuizRow, questions: QuestionRow[], choices: ChoiceRow[]): Quiz {
  return { id: row.id, title: row.title, description: row.description, coverImageUrl: row.cover_image_url, questions: questions.map((question) => ({
...
export async function getQuiz(id: string): Promise<Quiz | null> {
  const quizzes = await query<QuizRow[]>('SELECT id, title, description, cover_image_url FROM quizzes WHERE id = ?', [id])
...
async function writeQuiz(id: string, input: QuizInput, isUpdate: boolean): Promise<Quiz> {
  ...
  await transaction(async (connection) => {
    if (isUpdate) {
      ...
      await connection.execute('INSERT INTO quizzes (id, title, description, cover_image_url) VALUES (?, ?, ?, ?)', [id, input.title, input.description, input.coverImageUrl ?? null])
    }
