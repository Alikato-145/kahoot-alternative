...
export function QuestionEditor({ question, index, quizId, onPendingImage, onChange, onRemove }: Props) {
  ...
  return <fieldset className="rounded border border-slate-300 p-5 space-y-4">
    ...
    <label className="block font-medium">ลำดับคำถาม<input type="number" min={1} className="mt-1 block w-28 rounded border p-2" value={question.order ?? index + 1} onChange={(event) => onChange({ ...question, order: Math.max(1, Number(event.target.value) || 1) })} /></label>
