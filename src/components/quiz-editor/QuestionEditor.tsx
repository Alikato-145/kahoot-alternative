...
export function QuestionEditor({ question, index, quizId, onChange, onRemove }: Props) {
  ...
  return <fieldset className="rounded border border-slate-300 p-5 space-y-4">
    ...
    <ImageUpload quizId={quizId} label="รูปคำถาม" value={question.questionImageUrl ?? null} onChange={(questionImageUrl) => onChange({ ...question, questionImageUrl })} />
    ...
    <ImageUpload quizId={quizId} label="รูปเฉลย" value={question.revealImageUrl ?? null} onChange={(revealImageUrl) => onChange({ ...question, revealImageUrl })} />
