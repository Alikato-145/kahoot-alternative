...
describe('Socket.IO game protocol', () => {
  ...
  it('synchronizes a host and two players, including rank payloads', async () => {
    ...
    first.emit('player:answer', { pin: session.pin, playerId: 'first', questionId: 'socket-q', choiceId: 'a' })
    await firstAccepted
    const secondAccepted = once(second, 'answer:accepted')
    second.emit('player:answer', { pin: session.pin, playerId: 'second', questionId: 'socket-q', choiceId: 'b' })
    await secondAccepted
    const rank = once(first, 'score:rank-update')
    host.emit('host:reveal', { sessionId: session.id })
    expect(await rank).toMatchObject({ playerId: 'first', previousRank: 1, rank: 1, totalScore: expect.any(Number) })
